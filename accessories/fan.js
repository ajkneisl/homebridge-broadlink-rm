...
class FanAccessory extends BroadlinkRMAccessory {
    ...
    setDefaults() {
        ...
        config.simpleSpeed = config.simpleSpeed || 1;

        if (config.simpleSpeed === 1) {
            this.currentSpeed = 0;
    ...
    async setFanSpeed(hexData) {
        ...
        // if using simple speed up / speed down :)
        if (config.simpleSpeed) {
            const diff = state.fanSpeed - this.currentSpeed
            const hexCode = data[diff > 0 ? "speedDown" : "speedUp"]

            await this.performSend(Array(Math.abs(diff)).fill(hexCode));
