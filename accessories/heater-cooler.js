...
class HeaterCoolerAccessory extends BroadlinkRMAccessory {
    ...
    updateServiceCurrentHeaterCoolerState() {
        ...
        if (logLevel <= 2) {
            log(`Updated currentHeaterCoolerState to ${state.currentHeaterCoolerState}`)
        }
    }
    ...
    async setTargetHeaterCoolerState(hexData, previousValue) {
        ...
        if (logLevel <= 2) {
            log(`Changing target state from ${previousValue} to ${targetHeaterCoolerState}`)
        }
        switch (targetHeaterCoolerState) {
            case Characteristic.TargetHeaterCoolerState.COOL:
            ...
            default:
                if (logLevel <= 4) {
                    log(`BUG: ${this.name} setTargetHeaterCoolerState invoked with unsupported target mode ${targetHeaterCoolerState}`)
                }
        }
    ...
    decodeTemperatureHex(temperature, hexDataObject, toUpdateCharacteristic) {
        ...
        if (temperatureHexDataObject) {
            ...
            if (logLevel <= 4) {
                log(`\tDid not find temperature code for ${temperature}. Please update data.${this.state.targetHeaterCoolerState === 1 ?
    ...
    async setSwingMode(hexData, previousValue) {
        ...
        if (logLevel <= 2) {
            log(`${name} setSwingMode: Changing swing from ${previousValue} to ${Characteristic.SwingMode.SWING_ENABLED}`)
        }
    ...
    async setRotationSpeed(hexData, previousValue) {
        ...
        if (logLevel <= 2) {
            log(`${name} setRotationSpeed: Changing RotationSpeed from ${previousValue} to ${state.rotationSpeed}`)
        }
    ...
    async monitorTemperature() {
        ...
        if (w1DeviceID || temperatureFilePath) {
            config.temperatureUpdateFrequency = Math.max(config.temperatureUpdateFrequency, 60);
        }
        ...
        if (!config.isUnitTest) {
            setInterval(this.updateTemperatureUI.bind(this), config.temperatureUpdateFrequency * 1000)
        }
    }
    ...
    onTemperature(temperature, humidity) {
        ...
        if (config.noHistory !== true && this.state.currentTemperature != 0.00) {
            ...
            if (logLevel <= 1) {
                log(`\x1b[34m[DEBUG]\x1b[0m ${name} Logging data to history: temp: ${this.state.currentTemperature}, humidity: ${this.state.currentHumidity}`);
            }
            if (noHumidity) {
    ...
    addTemperatureCallbackToQueue(callback) {
        ...
        if (Object.keys(this.temperatureCallbackQueue).length > 1) {
            if (state.currentTemperature) {
                if (logLevel <= 1) {
                    log(`\x1b[34m[DEBUG]\x1b[0m ${name} addTemperatureCallbackToQueue (clearing previous callback, using existing temperature)`);
                }
        ...
        device.checkTemperature();
        if (logLevel < 1) {
            log(`\x1b[34m[DEBUG]\x1b[0m ${name} addTemperatureCallbackToQueue (requested temperature from device, waiting)`);
        }
    }
    ...
    updateTemperatureFromFile() {
        ...
        if (logLevel <= 1) {
            log(`\x1b[34m[DEBUG]\x1b[0m ${name} updateTemperatureFromFile reading file: ${temperatureFilePath}`);
        }
        ...
        fs.readFile(temperatureFilePath, 'utf8', (err, data) => {
            ...
            if (logLevel <= 1) {
                log(`\x1b[34m[DEBUG]\x1b[0m ${name} updateTemperatureFromFile (parsed temperature: ${temperature} humidity: ${humidity})`);
            }
    ...
    updateTemperatureFromW1() {
        ...
        if (logLevel <= 1) {
            log(`\x1b[34m[DEBUG]\x1b[0m ${name} updateTemperatureFromW1 reading file: ${fName}`);
        }
        ...
        fs.readFile(fName, 'utf8', (err, data) => {
            ...
            if (logLevel <= 1) {
                log(`\x1b[34m[DEBUG]\x1b[0m ${name} updateTemperatureFromW1 (parsed temperature: ${temperature})`);
            }
    ...
    updateTemperatureUI() {
        ...
        if (!noHumidity) {
            serviceManager.updateCharacteristic(Characteristic.CurrentRelativeHumidity, state.currentHumidity);
        }
    }
    ...
    getCurrentTemperature(callback) {
        ...
        if (defaultNowTemperature !== undefined) {
            if (logLevel <= 1) {
                log(`\x1b[34m[DEBUG]\x1b[0m ${name} getCurrentTemperature (using defaultNowTemperature ${defaultNowTemperature} from config)`);
            }
    ...
    onMQTTMessage(identifier, message) {
        ...
        if (objectFound) {
            ...
            if (logLevel <= 1) {
                log(`\x1b[34m[DEBUG]\x1b[0m ${name} onMQTTMessage (parsed value: ${value})`);
            }
